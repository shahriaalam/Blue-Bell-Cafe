...
**For Next.js App Router / React Templates:**
1. Copy `references/scrub-engine.js` into `public/scrub-engine.js`.
2. Save generated video/image assets in `public/assets/`.
3. Create or update a client component `components/LetsScrollWorld.tsx`:
```tsx
'use client';
import { useEffect, useRef } from 'react';

export default function LetsScrollWorld({ config }: { config: any }) {
  const containerRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (typeof window === 'undefined' || !containerRef.current) return;
    const init = () => {
      if ((window as any).mountLetsScroll && containerRef.current) {
        containerRef.current.innerHTML = '';
        (window as any).mountLetsScroll(containerRef.current, config);
      }
    };
    if ((window as any).mountLetsScroll) {
      init();
    } else {
...
```
4. In `app/page.tsx`, import `<LetsScrollWorld config={config} />` and pass the generated sections and connectors array.
